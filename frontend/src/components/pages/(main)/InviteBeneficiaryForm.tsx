...
export type InviteBeneficiaryFormProps = {
  /** Notified once the invite URL is ready. Onboarding wrapper uses this to enable a Continue button. */
  onReady?: (url: string) => void;
};

export function InviteBeneficiaryForm({ onReady }: InviteBeneficiaryFormProps) {
  ...
  useEffect(() => {
    if (state.status === "ready") onReady?.(state.url);
  }, [state, onReady]);
    ...
    <div className="flex flex-col gap-6">
      <Card>
        <CardContent>
          ...
          <CardDescription>
            The wallet you named in your will. The invite token is bound to this
            address — when the recipient clicks it, the bot pairs <em>their</em>{" "}
            chat with <em>their</em> wallet (not yours).
          </CardDescription>
        ...
        <CardContent>
          ...
          <ul className="mt-2 flex flex-col gap-3 font-apple text-[14px] text-[#1a1a1a]/70">
            <li>
              ...
              start the chat first. Clicking the invite link does exactly that.
            </li>
            <li>
              • The token is one-time and bound to the beneficiary&apos;s wallet
              — even if the link leaks, only their chat can bind to that wallet
              (others see &quot;already linked&quot;).
            </li>
            <li>
              • Once bound, the bot DMs them WillExecuted notifs + AI-generated
              farewell + a{" "}
